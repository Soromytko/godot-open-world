...
func _insert_random_placeform(pos : Vector3 = Vector3.ZERO):
	var rng := RandomNumberGenerator.new()
	var placeform := class_placeform.new()
	#rng.seed = hash(Time.get_time_dict_from_system())
	var half_size := _plant.size / 2
	placeform.transform.origin = pos
	if pos == Vector3.ZERO:
		placeform.transform.origin = Vector3(
			rng.randf_range(-half_size.x, half_size.x),
			rng.randf_range(-half_size.y, half_size.y),
			rng.randf_range(-half_size.z, half_size.z)
		)
	_root_node.try_insert(placeform)
...
func _update_lod_recursively(octree_node : class_octree_node, observer_position : Vector3):
	octree_node.update_lod(observer_position)
...
func _init_root_node():
	...
	if maybe_root_node != null && maybe_root_node is class_octree_node:
		...
		_root_node.depth = 0
		_root_node.bounds = _create_root_node_bounds()
...
func _create_root_node_bounds() -> AABB:
	return AABB(-_plant.size / 2, _plant.size)
